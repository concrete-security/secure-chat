 ...
 * @param {string} options.proxyUrl - WebSocket proxy URL
 * @param {string} options.targetHost - Target TEE server
 * @param {Object} options.policy - Verification policy
 * @param {string} [options.serverName] - TLS server name
 * @param {Function} [options.onAttestation] - Callback when attestation is received
 * @returns {Promise<Object>} The attestation result
 */
export async function warmupConnection(options) {
  const { proxyUrl, targetHost, serverName, onAttestation, policy } = options;

  if (!proxyUrl || !targetHost) {
    throw new Error("proxyUrl and targetHost are required");
  }
  if (!policy) {
